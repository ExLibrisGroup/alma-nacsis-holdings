        ...
        if (!this.isEmpty(this.record.SYSDEPT)) {
            for (let i = 0; i < this.record.SYSDEPT.length; i++) {
                ...
                if (!this.isEmpty(this.record.SYSTEL) && this.record.SYSTEL.length >= i) {
                    fieldsArray.push(new ViewFieldBuilder().label("ILL.MemberInfo.SystemPhoneNum").content(this.record.SYSTEL[i]).build());
                }
                if (!this.isEmpty(this.record.SYSFAX) && this.record.SYSFAX.length >= i) {
                    fieldsArray.push(new ViewFieldBuilder().label("ILL.MemberInfo.SystemFax").content(this.record.SYSFAX[i]).build());
                }
